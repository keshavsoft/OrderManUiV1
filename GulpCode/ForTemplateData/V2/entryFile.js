...
const preparePrimarykeyTablesArray = ({ inPrimarykeyTableNamesAsArray }) => {
    const returnArray = inPrimarykeyTableNamesAsArray.map(element => {
        // const findPrimaryTable1 = CommonTables.tables.find(element => element.tableName === element);
        const findPrimaryTable = CommonTables.tables.find(
            ({ tableName }) => tableName === element
        );

        return {
            tableName: element,
            columns: findPrimaryTable?.columns
        };
    });
